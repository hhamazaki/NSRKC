...
model_data::model_data(int argc,char * argv[]) : ad_comm(argc,argv)
{
 ...
 cout << "SDRec " << endl << SDRec << endl;
 cout << "latag " << endl << latag << endl;  
 cout << "st_est " << endl << st_est << endl;
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  ewcpue.allocate(1,nyw,"ewcpue");
    ...
    ewcpue.initialize();
  #endif
  molp.allocate(1,na,fyear,lyear,"molp");
  #ifndef NO_AD_INITIALIZE
    molp.initialize();
  #endif
...
void model_parameters::convert_parameters_into_rates(void)
{
 ...
 // calculate molting and selectivity functions   
  ...
  {	
     L = (double(j)-1.0)*slt;
     L += slm;
     mp1(j) = 1.0-1.0/(1.0+mfexp(mol*(slm-L)+log(1.0/0.001-1.0)));  // molting probability : reverse logistic function// 
	 molp(j,fyear) = 1.0-1.0/(1.0+mfexp(mol*(slm-L)+log(1.0/0.001-1.0)));  // molting probability : reverse logistic function// 
 ...
 for (i=fyear;i<=lyear-1;i++)
  {
	 mol *= mfexp(log_mol_dev(i));
  for (j=1;j<=na;j++)
    {	
     L = (double(j)-1.0)*slt;
     L += slm;
	 //molting probability 
	 molp(j,i+1) = 1.0-1.0/(1.0+mfexp(mol*(slm-L)+log(1.0/0.001-1.0)));  
	}	  
  } 
   for (i=fyear;i<=lyear;i++)
    {
...
void model_parameters::get_number_by_size(void)
{
  ...
  bcw(i) = 0.0;
    ...
    for (j=1;j<=na;j++)
     {
      pp = 0.0; 
      for (k=1;k<=j;k++) pp += tgr(k,j)*tt0(k)*molp(k,i); //Each crab molts right after fishery ended      
      npw(j,i+1) = pp*exp(-(0.583-ys(i))*Mn(j)) + rpp(j)*rec(i);   //New shell crab molted + recruits 
      opw(j,i+1) = tt0(j)*(1.0-molp(j,i))*exp(-(0.583-ys(i))*Mn(j));  //Old shell crab are unmolted crab  
     }
   }
 for (i=fyear; i<=lyear; i++)
   {
...
void model_parameters::evaluate_the_objective_function(void)
{
  ...
  test = -(sum(elem_prod(so,rowsum(elem_prod(trans(ooo),trans(log(eoo+1.e-3)))))) - sum(elem_prod(so,rowsum(elem_prod(trans(ooo),trans(log(ooo+1.e-3)))))));   
