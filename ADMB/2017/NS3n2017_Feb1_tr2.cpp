 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  log_alpha.allocate(0.5,5.0,-smol,"log_alpha");
...
void model_parameters::initializationfunction(void)
{
  ...
  log_beta.set_initial_value(-2.5);
  ...
  rpp.initialize();				
   ...
   beta = mfexp(log_beta);
    if(smol>=1)
	 ...
	 {
	 if(mol2p >= 1) mp1 = 1.0/(1.0+mfexp(-alpha+beta*mlen));			 
 ...
 for (i=fyear;i<=lyear;i++)
  ...
  if(smol>=1) molp(i)=imol;
  else
    {
     ...
     molp(i) =  1.0/(1.0+mfexp(-alpha+beta*mlen));  	 
	 }
     else
	 {		
