...
void model_parameters::preliminary_calculations(void)
{
 ...
 cout << twsd << endl;
  ...
  for(i=1;i<=21;i++)
  {
        tagrecap1(tag_recov1(i,1),tag_recov1(i,2))=tag_recov1(i,3)+tag_recov2(i,3);
        tagrecap2(tag_recov1(i,1),tag_recov1(i,2))=tag_recov1(i,4)+tag_recov2(i,4);
        tagrecap3(tag_recov1(i,1),tag_recov1(i,2))=tag_recov1(i,5)+tag_recov2(i,5);
  }
...
void model_parameters::evaluate_the_objective_function(void)
{
  ...
  tf(4) = -(sum(elem_prod(st,colsum(elem_prod(ont+oot,log(ent+eot+1.e-3))))) - sum(elem_prod(st,colsum(elem_prod(ont+oot,log(ont+oot+1.e-3))))));   
  tf(6) = -lawp*(sum(elem_prod(sw,colsum(elem_prod(onw+oow,log(enw+eow+1.e-3))))) - sum(elem_prod(sw,colsum(elem_prod(onw+oow,log(onw+oow+1.e-3))))));       
  tf(8) = -(sum(elem_prod(sc,colsum(elem_prod(onc+ooc,log(enc+eoc+1.e-3))))) - sum(elem_prod(sc,colsum(elem_prod(onc+ooc,log(onc+ooc+1.e-3))))));      
  //Log likelhihood size proportion for summer fishery survey    
  tf(10) = -(sum(elem_prod(so,colsum(elem_prod(ono+ooo,log(eno+eoo+1.e-3))))) - sum(elem_prod(so,colsum(elem_prod(ono+ooo,log(ono+ooo+1.e-3))))));  
