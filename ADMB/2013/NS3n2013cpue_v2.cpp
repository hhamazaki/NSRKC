 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  npp.allocate(1,na-1,0.0,0.9,2,"npp");
  opp.allocate(1,na-1,0.0,0.9,2,"opp");
  r1.allocate(0.5,0.9,1,"r1");
  nop.allocate(0.5,0.9,2,"nop");
...
{
  int i,j;
  dvariable first_y, op, np, nf, ng;
  nf=0.0;
  ng=0.0;
  log_rec = log_relrec+log_recscale;
  first_y = mfexp(log_initpop);
   for (i=1;i<(ny-2);i++) rec(i) = mfexp(log_rec(i));
   ...
   op = first_y*(1-nop);
   nps(1,1) = np*npp(1);
   nps(2,1) = (np-nps(1,1))*npp(2);
   nps(3,1) = (np-nps(1,1)-nps(2,1))*npp(3);
   nps(4,1) = (np-nps(1,1)-nps(2,1)-nps(3,1))*npp(4);
   nps(5,1) = (np-nps(1,1)-nps(2,1)-nps(3,1)-nps(4,1))*npp(5);
   nps(6,1) = (np-nps(1,1)-nps(2,1)-nps(3,1)-nps(4,1)-nps(5,1));
   ops(2,1) = (op-ops(1,1))*opp(2);
   ops(3,1) = (op-ops(1,1)-ops(2,1))*opp(3);
   ops(4,1) = (op-ops(1,1)-ops(2,1)-ops(3,1))*opp(4);
   ops(5,1) = (op-ops(1,1)-ops(2,1)-ops(3,1)-ops(4,1))*opp(5);
   ops(6,1) = (op-ops(1,1)-ops(2,1)-ops(3,1)-ops(4,1)-ops(5,1));
   for (j=1;j<=na;j++){
  ...
  for (i=1;i<=nyt;i++)
   {
	...
	// ADFG survey has different adjustments.	
	if(it(i) > scy1) {ettq(i) = qtad*(ett1(i)+ett2(i));}
	  else
	  ettq(i) = qt*(ett1(i)+ett2(i));	  	 
   }
...
void model_parameters::evaluate_the_objective_function(void)
{
  ...
  f = tf1;
  tf2 = 0.5*norm2(log(tp+1.e-3)-log(qt*(etp1+etp2)+1.e-3))/log(spcv*spcv+1.0);
  f += tf2;
   T_var = sqrt(log(elem_prod(cvcpue,cvcpue)+1.0)+advar); 
   tf3 = sum(log(T_var))+0.5*(norm2(elem_div((log(stcpue+1.e-3)-log(1000*ecpue+1.e-3)),T_var))); 
   f += lamc*tf3;   
