...
void model_parameters::get_first_year_abundance(void)
{
  ...
  for (j=1;j<=na-2;j++) expo(j) = mfexp(flop(j));
  sn = sum(expn);
  so = sum(expo);
  for (j=1;j<=na-1;j++) npp(j) = expn(j)/(1+sn);      //f_year_p is the estimated proportions of the first year population abundance.
  ...
  opp(na) = 1-sum(opp(1,na-1));	
  for (j=1;j<=na;j++) nps(j,1) = npp(j)*np;
  for (j=1;j<=na;j++) ops(j,1) = opp(j)*op;
   for (j=1;j<=na;j++){
