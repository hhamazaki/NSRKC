...
void model_parameters::convert_parameters_into_rates(void)
{
 ...
 // calculate molitng and catchability function   
   ...
   selt(5) = 1.0/(1.0+mfexp(-1.0*st1*(118.5-st2)));
   selt(6) = 1.0/(1.0+mfexp(-1.0*st1*(128.5-st2)));    
  ...
  for (i=1;i<=nyt;i++)
   {
	...
	// trawl catchability adjusted  for all. 
...
{
  tf2 = 0.5*norm2(elem_div((log(tt+1.e-3)-log(ettq+1.e-3)),sqrt(log(elem_prod(cv3,cv3)+1.0))));
  f = tf2;
