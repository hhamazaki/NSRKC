 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  log_relrec.allocate(1,ny-1,-20.,20.,1,"log_relrec");
  ...
  eoo.allocate(1,na,1,nyo,"eoo");
    ...
    eoo.initialize();
  #endif
  enwp.allocate(1,na,1,ny-1,"enwp");
  #ifndef NO_AD_INITIALIZE
    enwp.initialize();
  #endif
  eowp.allocate(1,na,1,ny-1,"eowp");
  #ifndef NO_AD_INITIALIZE
    eowp.initialize();
  #endif
  enwpd.allocate(1,na,1,ny-1,"enwpd");
  #ifndef NO_AD_INITIALIZE
    enwpd.initialize();
  #endif
  eowpd.allocate(1,na,1,ny-1,"eowpd");
  #ifndef NO_AD_INITIALIZE
    eowpd.initialize();
  #endif
  enwc.allocate(1,na,1,ny-1,"enwc");
  #ifndef NO_AD_INITIALIZE
    enwc.initialize();
  #endif
  eowc.allocate(1,na,1,ny-1,"eowc");
  #ifndef NO_AD_INITIALIZE
    eowc.initialize();
  #endif
...
void model_parameters::preliminary_calculations(void)
{
  ...
  int i,j,Isim,iy,n0;
  dvariable ErrComm;
  dvariable tt0; // Calculated working variable does not need defferentiated  
  ...
  for (i=1;i<=ny;i++)
   {
    sc(i) *= efnw;
    if (sc(i) > maxsc) sc(i)=maxsc;             //summer fishery
   }
  for (i=1;i<=nyo;i++)
   {
    so(i) *= efnw;
    if (so(i) > maxsc) so(i)=maxsc;             //observer's data
	
   }
  for (i=1;i<=nyt;i++)
   {
    for (j=1;j<=na;j++) ont(j,i) += oot(j,i);
...
void model_parameters::convert_parameters_into_rates(void)
{
 ...
 // calculate molitng and selectivity functions   
   ...
   sel93(na) = 1.0;  
   selt = selt/selt(na-1);   
   selt(na) = 1.0;  //1.0 is 1.0 by default; however can be changed if we believe in dormshaped selectivity. 
   selp = selp/selp(na-1); 
   selp(na) = 1.0;   
   selw = selw/selw(na-1);
   selw(na) = sw3;   
...
void model_parameters::get_number_by_size(void)
{
    ...
    for (j=1;j<=na;j++)
     {
      ...
      npw(j,i) = pp*exp(-(0.583-ys(i))*Mn(j));
      if (j==1) npw(j,i) += r1*rec(i);
      if (j==2) npw(j,i) += (1.0-r1)*rec(i);
      opw(j,i) = tt0(j)*(1.0-mp0(j))*exp(-(0.583-ys(i))*Mn(j));
  ...
  last_legal = last_y(3)*lg(3)*wm(3)*sel93(3)+last_y(4)*lg(4)*wm(4)*sel93(4)+last_y(5)*lg(5)*wm(5)*sel93(5)+last_y(6)*lg(6)*wm(6)*sel93(6);
    ...
    for (j=1;j<=na;j++)
     {
      ...
      if (ent(j,i) < 0.0) ent(j,i) = 0.0;
		...
		if(it(i) < scy(1)) {ettq(i) = qtno*(ett1(i)+ett2(i));}
			else
		ettq(i) = (ett1(i)+ett2(i));
   }
