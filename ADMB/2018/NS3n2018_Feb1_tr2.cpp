 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  log_mol.allocate(-10.5,-1.0,-smol,"log_mol");
  log_alpha.allocate(-10.5,5.0,-smol,"log_alpha");
  log_beta.allocate(0.5,5.0,-smol,"log_beta");
  imol.allocate(1,na,0.,1.0,smol,"imol");
  log_mol_dev.allocate(fyear,lyear,-10.0,10.0,rmol_phase,"log_mol_dev");
  log_alpha_dev.allocate(fyear,lyear,-10.0,10.0,rmol_phase,"log_alpha_dev");
  log_beta_dev.allocate(fyear,lyear,-10.0,10.0,rmol_phase,"log_beta_dev");
  log_st1.allocate(1,2,-5.0,1.0,-sst,"log_st1");
  log_st2.allocate(1,2,-5.0,-0.5,-sst,"log_st2");
  ist.allocate(1,na,0.0,1.0,sst,"ist");
  ist2.allocate(1,na,0.0,1.0,sst,"ist2");
  log_sw1.allocate(-15.0,1.0,-ssw,"log_sw1");
  sw3.allocate(1,ra,0.,1.0,1,"sw3");
  isw.allocate(1,na,0.,1.0,ssw,"isw");
  log_sc1.allocate(1,3,-5.5,0.0,-ssc,"log_sc1");
  log_sc2.allocate(1,3,0.0,10.0,-ssc,"log_sc2");
  ...
  iig.allocate(1,na,0.,20.,sig,"iig");
  ...
  last_legalb.allocate("last_legalb");
  last_sublb.allocate("last_sublb");
...
void model_parameters::initializationfunction(void)
{
  ...
  log_mol.set_initial_value(-2.41358);
  log_alpha.set_initial_value(-2.7);
  log_beta.set_initial_value(4.5);
  log_st1.set_initial_value(-2.5);
  log_st2.set_initial_value(-2.0);
  log_sc1.set_initial_value(-2.7);
  log_sc2.set_initial_value(4.5);
  ...
  rpp.initialize();				
   ...
   beta = mfexp(log_beta);
    if(smol>=1)
	 ...
	 {
	 if(mol2p >= 1) mp1 = 1.0/(1.0+mfexp(alpha*(mlen-beta)));			 
     ...
     if(nsc==2) selc1(2) = isc2; // Estimate selectivity two periods individul length class	   		 
	  ...
	  {
    selc1(1) = 1.0/(1.0+mfexp(-sc1(1)*(mlen-sc2(1))));  // commercial pot selectivity 1976-1992: logistic function//
    selc1(2) = 1.0/(1.0+mfexp(-sc1(nsc)*(mlen-sc2(nsc))));  // commercial pot selectivity 1993 - : logistic function//		  
     selc1(1,na) = 1.0;
	 selc1(2,na) = 1.0;
	  }
	if(ssw>=1)  selw = isw;
