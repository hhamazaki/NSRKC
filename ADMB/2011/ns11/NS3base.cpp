 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  log_mo1.allocate(-4.15,-2.0,2,"log_mo1");
  log_mo2.allocate(0.55,5.00,2,"log_mo2");
  log_st1.allocate(-5.5,-1.0,1,"log_st1");
  log_st2.allocate(0.51,3.00,1,"log_st2");
  log_sp1.allocate(-3.7,-1.0,1,"log_sp1");
  log_sp2.allocate(2.00,5.00,1,"log_sp2");
  log_sw1.allocate(-3.0,0.11,1,"log_sw1");
  log_sw2.allocate(3.00,5.00,1,"log_sw2");
  sw3.allocate(0.1,1.0,1,"sw3");
  log_sc1.allocate(-4.5,-1.9,1,"log_sc1");
  log_sc2.allocate(4.35,6.75,1,"log_sc2");
  log_mo3.allocate(-3.5,-1.9,1,"log_mo3");
  log_mo4.allocate(4.35,6.75,1,"log_mo4");
  sos0.allocate(0.50,1.5,-1,"sos0");
  sow.allocate(0.300,1.7,-1,"sow");
...
void model_parameters::initializationfunction(void)
{
  ...
  log_mo3.set_initial_value(-2.41358);
  log_mo4.set_initial_value(4.82115);
...
{
  int i, j;
  dvariable mo1,mo2,mo3,mo4;                          // working variables: molting probability
  dvariable sc1,sc2,st1,st2,sp1,sp2,sw1,sw2;          // working variables
   ...
   for (j=1;j<=na;j++)
    {
     ...
     sel93(j) = 1.0/(1.0+mfexp(-1.0*mo3*(pp-mo4)));
    }
  ...
  for (i=1;i<ny;i++)
   {
    ...
    ttt0 = 0.0;
    bc(i) = 0.0;
    for (k=1;k<=na;k++) {ttt0 = ttt0 + (nps(k,i)+ops(k,i))*lg(k);}
    if (ttt0 < tc(i))
    { ttt0 = tc(i)*1.2;}
...
{
  dvariable tf1,tf2,tf3,tf4,tf5,tf6,tf7,tf9,tf11,tf13,tf14,tf15;
