 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  log_q1.allocate(-12.5,-8.5,1,"log_q1");
  log_q2.allocate(-12.5,-10.0,1,"log_q2");
  log_q3.allocate(-12.5,-10.0,1,"log_q3");
  log_initpop.allocate(2,15,1,"log_initpop");
  log_recscale.allocate(2,12,1,"log_recscale");
  log_relrec.allocate(1,ny-1,-15.,15.,1,"log_relrec");
  r1.allocate(0.5,0.9,1,"r1");
  log_mo1.allocate(-4.15,-2.0,1,"log_mo1");
  log_mo2.allocate(0.55,5.00,1,"log_mo2");
  log_st1.allocate(-5.5,-1.0,1,"log_st1");
  log_st2.allocate(0.51,3.00,1,"log_st2");
  log_st3.allocate(-5.5,-1.0,1,"log_st3");
  log_st4.allocate(0.51,3.00,1,"log_st4");
  log_sp1.allocate(-3.0,-1.00,1,"log_sp1");
  log_sp2.allocate(3.9,5.50,1,"log_sp2");
  log_sw1.allocate(-3.0,1.00,1,"log_sw1");
  log_sw2.allocate(3.90,5.50,1,"log_sw2");
  sw3.allocate(0.1,1.0,1,"sw3");
  log_sc1.allocate(-3.0,-1.00,1,"log_sc1");
  log_sc2.allocate(3.9,5.50,1,"log_sc2");
  log_sc3.allocate(-3.0,-1.00,1,"log_sc3");
  log_sc4.allocate(3.9,5.50,1,"log_sc4");
  ...
  ete.allocate(1,ny,"ete");
    ...
    ete.initialize();
  #endif
  ecp.allocate(1,ny,"ecp");
  #ifndef NO_AD_INITIALIZE
    ecp.initialize();
  #endif
  ecp3.allocate(1,ny,"ecp3");
  #ifndef NO_AD_INITIALIZE
    ecp3.initialize();
  #endif
  tb.allocate(1,ny,"tb");
...
void model_parameters::initializationfunction(void)
{
  ...
  log_q3.set_initial_value(-11.5);
  log_initpop.set_initial_value(7.0);
  r1.set_initial_value(0.6);
  log_mo1.set_initial_value(-2.41358);
  log_mo2.set_initial_value(4.8211);
  sw3.set_initial_value(1.0);
  log_sc1.set_initial_value(-2.00);
  log_sc2.set_initial_value(4.38);
  log_sc3.set_initial_value(-2.00);
  log_sc4.set_initial_value(4.38);
  log_sc5.set_initial_value(-2.00);
  log_sc6.set_initial_value(4.38);
...
void model_parameters::preliminary_calculations(void)
{
  ...
  for (i=2;i<=(ny-1);i++) ocp(i) = tc(i)/te(i);
	ocp(ny) = 0.00;
	ocp(1) = 0.00;
	ocp(16) = 0.00;
 // ocp3(1) = ocp(1);
 // ocp3(2) = ocp(2);
 // ocp3(3) = ocp(3);
  for (i=1;i<=17;i++) ocp3(i) = 0.0;
  //	ocp3(i) = (ocp(i-2)+ocp(i-1)+ocp(i))/3;
 // ocp3(17) = (ocp(15)+ocp(17))/2;
  ocp3(18) = (ocp(17)+ocp(18))/2;
  for (i=19;i<=32;i++) 
	ocp3(i) = (ocp(i-2)+ocp(i-1)+ocp(i))/3; 
  ocp3(33) = ocp(33);
  ocp3(34) = (ocp(33)+ocp(34))/2;
  for (i=35;i<=(ny-1);i++) 	
	ocp3(i) = (ocp(i-2)+ocp(i-1)+ocp(i))/3; 	 
  ocp3(ny) = 0.00; 
...
{
  int i, j;
  dvariable mo1,mo2;                          // working variables: molting probability
  dvariable sc1,sc2,sc3,sc4, sc5, sc6, st1,st2, st3, st4, sp1,sp2,sw1,sw2;          // working variables: Selectivity 
  dvariable pp;                                       // local variable
   //catchability
   q1 = mfexp(log_q1);
   q2 = mfexp(log_q2);
   q3 = mfexp(log_q3);
...
void model_parameters::get_number_by_size(void)
{
  ...
  for (i=1;i<ny;i++)
   {
    ...
    if (i < 7)
     ...
     if (i < 16)
      ...
      mp0 = mp3;
    // Survivors from the summer fishery and calculate discareds 
    ttt0 = 0.0;
    bc(i) = 0.0;
	// ttt0: total number of legal crab  
	...
	if (i >= 29) bc(i) += tc(i)/ttt0*(nps(4,i)+ops(4,i))*sel(4,i)*(1.0-lg(k)*(p4-1))*hm*wm(4);
    // Dynamics
    for (j=1;j<=na;j++)
     {
      pp = 0;
	...
	if (i >= 29) tb(i+1) += (p4-1)*(nps(4,i+1)+ops(4,i+1)*sos(4))*sel(4,i+1)*lg(4);
	...
	// Since 2005 (i=29) Commercial accept only > CW 5 inch crab, which affects only 4th crab size.	 	 
	if (i >= 29)
		{
  ...
  // last_legal = abundance*proportion of legal*catchability*average weight
   last_legal = last_y(3)*lg(3)*wm(3)*sel08(3)+last_y(4)*lg(4)*p4*wm(4)*sel08(4)+last_y(5)*lg(5)*wm(5)*sel08(5)+last_y(6)*lg(6)*wm(6)*sel08(6);
...
void model_parameters::evaluate_the_objective_function(void)
{
 ...
 //weight each year estimate by 1/(2*variance) - use cv of biomass in sqrt(log(cv^2+1)) as sd of log(biomass)
  tf2 = 0.5*norm2(elem_div((log(tt+1.e-3)-log(ett1+ett2+1.e-3)),sqrt(log(elem_prod(cv3,cv3)+1.0))));
  f += tf2;
  tf3 = 0.5*norm2(log(tp+1.e-3)-log(etp1+etp2+1.e-3))/log(0.34*0.34+1.0);
  f += tf3;
...
void model_parameters::report()
{
 adstring ad_tmp=initial_params::get_reportfile_name();
  ...
  report << "ecp" << endl << ecp << endl;  // Estimated Summer fishery cpue
