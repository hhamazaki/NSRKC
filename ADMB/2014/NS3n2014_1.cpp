 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  log_q1.allocate(-32.5,10.0,1,"log_q1");
  ...
  log_relrec.allocate(1,ny,-20.0,30.0,1,"log_relrec");
  ...
  log_st2.allocate(0.51,6.5,1,"log_st2");
  log_sw1.allocate(-10.0,1.0,2,"log_sw1");
  log_sw2.allocate(3.90,5.50,2,"log_sw2");
  sw3.allocate(0.1,1.0,1,"sw3");
  log_sc1.allocate(-4.0,1.0,1,"log_sc1");
  log_sc2.allocate(3.9,10.5,1,"log_sc2");
  log_sc3.allocate(-3.0,1.0,1,"log_sc3");
  log_sc4.allocate(3.9,10.5,1,"log_sc4");
  log_sc5.allocate(-3.0,1.0,1,"log_sc5");
  log_sc6.allocate(3.9,10.5,1,"log_sc6");
...
void model_parameters::get_number_by_size(void)
{
  ...
  last_mmb = 0.0;
  for (j=3; j<=na; j++) last_mmb += (npw(j,ny+1)+opw(j,ny+1))*wm(j)*exp(-0.417*Mn(j));
