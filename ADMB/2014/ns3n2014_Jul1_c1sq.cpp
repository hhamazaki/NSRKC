...
#include <ns3n2014_Jul1_c1sq.htp>
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  log_q1.allocate(-10.5,20.0,1,"log_q1");
  log_q3.allocate(-10.5,20.0,1,"log_q3");
  ...
  log_relrec.allocate(1,ny-1,-15.0,15.0,1,"log_relrec");
  flnp.allocate(1,na-1,-5.0,5.0,1,"flnp");
  r1.allocate(0.5,0.9,1,"r1");
  log_mo1.allocate(-5.5,-1.0,2,"log_mo1");
  log_st1.allocate(-10.0,1.0,2,"log_st1");
  log_st3.allocate(-10.0,1.0,2,"log_st3");
  log_sw1.allocate(-10.0,1.0,2,"log_sw1");
  sw3.allocate(0.1,1.000001,2,"sw3");
  log_sc1.allocate(-15.0,1.0,2,"log_sc1");
  log_sc5.allocate(-15.0,1.0,2,"log_sc5");
  advar.allocate(0.0,6.0,2,"advar");
  advarw.allocate(0.0,10.0,2,"advarw");
  qtno.allocate(0.1,1.0,qtno_phase,"qtno");
  M.allocate(0.1,0.5,M_phase,"M");
  ms.allocate(1.0,5.0,ms_phase,"ms");
  sigma.allocate(0.,30.,1,"sigma");
 ...
 fbest = 1.0e+20;
  ...
  legaln.allocate(1,ny+1,"legaln");
  legalb.allocate(1,ny+1,"legalb");
...
void model_parameters::initializationfunction(void)
{
  ...
  log_mo1.set_initial_value(-5.41358);
  log_st1.set_initial_value(-9.0);
  log_st3.set_initial_value(-9.0);
  log_sc1.set_initial_value(-2.31267);
  log_sc5.set_initial_value(-2.5);
  log_sw1.set_initial_value(-8.0);
  sw3.set_initial_value(0.99);
...
void model_parameters::convert_parameters_into_rates(void)
{
 ...
 //      mp1(j) = 1.0;
     ...
     sel08(j) = 1.0/(1.0+mfexp(sc5*(Lm-L)+log(1.0/0.999-1.0)));  // commercial pot selectivity 1993 - : logistic function//
     ...
     selt2(j) = 1.0/(1.0+mfexp(st3*(Lm-L)+log(1.0/0.999-1.0)));   // ADFG summer trawl net selectivity: 1996- logistic function//     
    }
   selw(na) = sw3;   
   for (i = 1; i<=ny; i++)
    {
  ...
  ef.initialize();
   for (i=1;i<=(na-1);i++)
   {
    for (j=i;j<=na;j++)
     ...
     fa = exp(-0.5*square((slt*(j-1.5)-ig(1)-ig(2)*i)/sigma));
     fb = exp(-0.5*square((slt*(j-0.5)-ig(1)-ig(2)*i)/sigma));
     fab = exp(-0.5*square((slt*(j-1)-ig(1)-ig(2)*i)/sigma));
...
void model_parameters::get_number_by_size(void)
{
  ...
  for (i=1;i<= ny;i++)
  {
    ...
    for (j=1;j<=na;j++)
    {
        tb(i) += (nps(j,i)+ops(j,i)*sos(j))*sel(j,i)*lg(j);  // The number of legal crab catchable to commercial fisheries	
    }
	 for (j=1;j<=na;j++)
     {
      enc(j,1) = 0.0;
    ...
    for (k=1;k<=na;k++)
     {
      ...
      bc(i) += (tc(i)/ttt0)*(nps(k,i)+ops(k,i))*sel(k,i)*(1.0-lg(k))*hm(1)*wm(k);
     }
    for (j=1;j<=na;j++)
     {
      pp = 0;
      for (k=1;k<=j;k++) pp += gr(k,j)*tt0(k)*mp0(k); //Each crab molts right after fishery ended  
      npw(j,i) = pp*exp(-(0.583-ys(i))*Mn(j));  //New shell crab on Feb 1st 
      if (j==1) npw(j,i) += r1*rec(i);
      if (j==2) npw(j,i) += (1.0-r1)*rec(i);
      opw(j,i) = tt0(j)*(1.0-mp0(j))*exp(-(0.583-ys(i))*Mn(j)); //Old shell crab are unmolted crab
     }
    ...
    for (j=1;j<=na;j++)
	{
     nps(j,i+1) = (npw(j,i)-enwp(j,i)*tws(i)-enwc(j,i)*twc(i)-(twc(i)/tt1)*(npw(j,i))*selw(j)*(1-lg(j))*hm(2)-twsd(i)*enwpd(j,i)*hm(2))*exp(-0.417*Mn(j));  
     ops(j,i+1) = (opw(j,i)-eowp(j,i)*tws(i)-eowc(j,i)*twc(i)-(twc(i)/tt1)*(opw(j,i))*selw(j)*(1-lg(j))*hm(2)-twsd(i)*eowpd(j,i)*hm(2))*exp(-0.417*Mo(j));	
...
void model_parameters::evaluate_the_objective_function(void)
{
  tf(1) = 0.5*norm2(elem_div((log(tt+1.e-3)-log(ettq+1.e-3)),sqrt(log(elem_prod(cv3,cv3)+1.0))));
   tf(2) = lamw*norm2(log(wcpue+1.e-3)-log(ewcpue+1.e-3))/(2*advarw*advarw); 
  ...
  tf(5) = -lawp*(sum(elem_prod(sw,colsum(elem_prod(onw+oow,log(enw+eow+1.e-3))))) - sum(elem_prod(sw,colsum(elem_prod(onw+oow,log(onw+oow+1.e-3))))));      
  tf(6) = -(sum(elem_prod(sc,colsum(elem_prod(onc+ooc,log(enc+eoc+1.e-3))))) - sum(elem_prod(sc,colsum(elem_prod(onc+ooc,log(onc+ooc+1.e-3))))));      
  tf(7) = -(sum(elem_prod(so,colsum(elem_prod(ono+ooo,log(eno+eoo+1.e-3))))) - sum(elem_prod(so,colsum(elem_prod(ono+ooo,log(ono+ooo+1.e-3))))));   
  tf(8) = norm2(log_relrec)/(2*SDRec*SDRec);                            
...
void model_parameters::Proj1Yr(void)
{
  ...
  for (Len=1;Len<=na;Len++)
   {
    pp = 0;
    for (Len1=1;Len1<=Len;Len1++) pp += tgr(Len1,Len)*tt0(Len1)*mp0(Len1);
    npw(Len,YrPass) = pp;
...
void model_parameters::report()
{
 adstring ad_tmp=initial_params::get_reportfile_name();
  ...
  report << "ecpue" << endl << ecpue << endl;  // Estimated Summer fishery cpue//  Estimated Summer fishery cpue  
