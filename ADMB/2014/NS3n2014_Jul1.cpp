 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  log_q1.allocate(-10.5,10.0,1,"log_q1");
  log_q3.allocate(-10.5,10.0,1,"log_q3");
  log_qw.allocate(-10.5,10.0,1,"log_qw");
  log_initpop.allocate(2.0,15.0,1,"log_initpop");
  log_recscale.allocate(2.0,12.0,1,"log_recscale");
  log_relrec.allocate(1,ny-1,-11.0,25.0,1,"log_relrec");
  flnp.allocate(1,na-1,-5.0,5.0,1,"flnp");
  r1.allocate(0.5,0.9,1,"r1");
  log_mo1.allocate(-5.5,-2.0,2,"log_mo1");
  log_mo2.allocate(0.55,10.0,2,"log_mo2");
  log_st1.allocate(-10.0,1.0,2,"log_st1");
  log_st2.allocate(0.51,10.0,2,"log_st2");
  log_st3.allocate(-10.0,1.0,2,"log_st3");
  log_st4.allocate(0.51,10.0,2,"log_st4");
  log_sw1.allocate(-10.0,1.0,2,"log_sw1");
  log_sw2.allocate(3.90,5.50,2,"log_sw2");
  sw3.allocate(0.1,1.0,2,"sw3");
  log_sc1.allocate(-5.0,1.0,2,"log_sc1");
  log_sc2.allocate(3.9,7.5,2,"log_sc2");
  log_sc5.allocate(-5.0,1.0,2,"log_sc5");
  log_sc6.allocate(3.9,7.5,2,"log_sc6");
...
void model_parameters::initializationfunction(void)
{
  ...
  log_st2.set_initial_value(5.00);
  log_st3.set_initial_value(-2.5);
  log_st4.set_initial_value(5.00);
   ...
   for (j=1;j<=na;j++)
    {
     ...
     selt1(j) = 1.0/(1.0+mfexp(-1.0*st1*(pp-st2)));   // summer trawl net selectivity: logistic curve//
     selt2(j) = 1.0/(1.0+mfexp(-1.0*st3*(pp-st4)));   // summer trawl net selectivity: logistic curve//     
    }
   selw(na) = sw3;   
   for (i = 1; i<=ny; i++)
    {
...
void model_parameters::evaluate_the_objective_function(void)
{
  ...
  f = tf1;
   tf2 = norm2(log(wcpue+1.e-3)-log(ewcpue+1.e-3))/(2*log(SDW*SDW+1)); 
   f += lamw*tf2;  
  ...
  f -= tf7;
  tf8 = norm2(log_relrec)/(2*SDRec*SDRec);                            //deviation in recruits.
  f += tf8;
