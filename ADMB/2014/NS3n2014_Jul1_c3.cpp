...
void model_parameters::evaluate_the_objective_function(void)
{
  ...
  tf(5) = -lawp*(sum(elem_prod(sw,colsum(elem_prod(onw,log(enw+1.e-3))))) - sum(elem_prod(sw,colsum(elem_prod(onw,log(onw+1.e-3))))));     
 tf(6) = -lawp*(sum(elem_prod(sw,colsum(elem_prod(oow,log(eow+1.e-3))))) - sum(elem_prod(sw,colsum(elem_prod(oow,log(oow+1.e-3))))));       
  tf(7) = -(sum(elem_prod(sc,colsum(elem_prod(onc,log(enc+1.e-3))))) - sum(elem_prod(sc,colsum(elem_prod(onc,log(onc+1.e-3))))));      
  //Log likelhihood size proportion for summer fishery survey    
  tf(8) = -(sum(elem_prod(sc,colsum(elem_prod(ooc,log(eoc+1.e-3))))) - sum(elem_prod(sc,colsum(elem_prod(ooc,log(ooc+1.e-3))))));      
  tf(9) = -(sum(elem_prod(so,colsum(elem_prod(ono,log(eno+1.e-3))))) - sum(elem_prod(so,colsum(elem_prod(ono,log(ono+1.e-3))))));   
  tf(10) = -(sum(elem_prod(so,colsum(elem_prod(ooo,log(eoo+1.e-3))))) - sum(elem_prod(so,colsum(elem_prod(ooo,log(ooo+1.e-3))))));   
  tf(11) = norm2(log_relrec)/(2*SDRec*SDRec);                            
