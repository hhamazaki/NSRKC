 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  log_relrec.allocate(1,ny-1,-20.0,20.0,1,"log_relrec");
   ...
   for (j=1;j<=na;j++)
    {
     L = (double(j)-1.0)*slt;
     L += slm;
     mp1(j) = 1.0-1.0/(1.0+mfexp(mo1*(slm-L)+log(1.0/0.01-1.0)));  // molting probability : reverse logistic function//
     sel92(j) = 1.0/(1.0+mfexp(sc1*(Lm-L)+log(1.0/0.99-1.0)));  // commercial pot selectivity 1976-1992: logistic function//
     sel08(j) = 1.0/(1.0+mfexp(sc5*(Lm-L)+log(1.0/0.99-1.0)));  // commercial pot selectivity 1993 - : logistic function//
     selw(j) = 1.0/(1.0+mfexp(sw1*(Lm-L-slt)+log(1.0/0.99-1.0)));   // winter pot selectivity 1981-2011: logistic function//
     selt1(j) = 1.0/(1.0+mfexp(st1*(Lm-L)+log(1.0/0.99-1.0)));   // NOAA summer trawl net selectivity 1976-1992: logistic function//
     selt2(j) = 1.0/(1.0+mfexp(st3*(Lm-L)+log(1.0/0.99-1.0)));   // ADFG summer trawl net selectivity: 1996- logistic function//     
    }
   selw(na) = sw3;   
   for (i = 1; i<=ny; i++)
    {
  ...
  bcw(i) = 0.0;
    ...
    for (k=1;k<=na;k++)
     {	  
      ...
      grt(k) = gr(k,1)+gr(k,2)+gr(k,3)+gr(k,4)+gr(k,5)+rm*gr(k,6);
     }
	 for (j=1;j<=na;j++)
        {
        for (k=1;k<=na;k++){
        grc(j,k) = gr(j,k)/grt(j);
        }
        grc(j,na)=grc(j,na)*rm;
       }
    for (j=1;j<=na;j++)
     {
      pp = 0.0; 
      for (k=1;k<=j;k++) pp += grc(k,j)*tt0(k)*mp0(k); //Each crab molts right after fishery ended      
...
void model_parameters::report()
{
 adstring ad_tmp=initial_params::get_reportfile_name();
  ...
  report << "grc" << endl << grc << endl;
  report << "sw3" << endl << sw3 << endl;
}
