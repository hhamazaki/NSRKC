//NS3n2014cpue_1.TPL
//This model is a NS3n2013_rev.TPL model adopted by the CPT in 2013
//The model incorporated cpue instead of effort
//The model also incorporated trawl catchability qt:
//CPUE likelihood was calculated with additional variance
//Summer pot survey likelihood has weight (wsp and wspl) that can be turned on and off.
//The initial length proportion was estimated.
//The model changes model starting period changed to Feb 1st
...
DATA_SECTION
  ...
  init_ivector scy(1,2)           //summer commercial fishery event indices: 1)year trawl fishery ended, 2) escape mechanism installed
  ...
  init_number qta			  // trawl catchability multiplier
  ...
  init_vector tsc(1,ny)     //annual retained catch for winter subsistence fishery
  init_vector tsct(1,ny)     //annual total catch for winter subsistence fishery
  ...
  init_int qtno_phase         // Survey Q phase control vector (NOAA, ADF&G)
  init_int qtad_phase         // Survey Q phase control vector (NOAA, ADF&G)
...
//===============================================================================================================
PARAMETER_SECTION
  init_bounded_number log_q1(-32.5,8.5,1)     //Commercial firherey Catchability 1977-1992
  init_bounded_number log_q2(-32.5,10.0,1)    //Commercial firherey Catchability 1993-2007
  init_bounded_number log_q3(-32.5,10.0,1)   //Commercial firherey Catchability after 2008
  init_bounded_number log_initpop(2,15,1) //Initial population size
 init_bounded_number log_recscale(2,12,2)
  init_bounded_dev_vector log_relrec(1,ny,-20.,30.,2)
  init_bounded_vector flnp(1,na-1,-5.,5.,1)     //Initial year length proportion (log scale)
  ...
  init_bounded_number log_mo2(0.55,10.0,1)       //length at 50% molting probability
  init_bounded_number log_st1(-10.0,-1.0,1)      //parameter for selectivity: NOAA trawl survey
  init_bounded_number log_st2(0.51,10.0,1)      //length at 50% selectivity: NOAA trawl survey
...
//  init_bounded_number log_sp2(3.9,5.50,1)      //length at 50% selectivity: pot survey : Not used
  init_bounded_number log_sw1(-10.0,10.00,1)      //parameter for selectivity: winter project
  ...
  init_bounded_number log_sc5(-3.0,-1.00,1)      //parameter for selectivity: summer fishery after 2008: Not used
  init_bounded_number log_sc6(3.9,5.50,1)      //length at 50% selectivity: summer fishery after 2008: Not used
  init_bounded_number advar(0.0,6.0,2)         //selectivity of old-shell crabs in winter project
  init_bounded_number qtno(0.1,2.5,qtno_phase)           // Catchability of NOAA trawl survey
  init_bounded_number qtad(0.1,2.5,qtad_phase)           // Catchability of ADF&G trawl survey
//  init_bounded_number M(0.1,0.3,M_phase)           //Mortality  Does not use
//  init_bounded_number ms(1.0,5.0,ms_phase)         //Mortality multiplier for the last length group
  init_bounded_number p4(0.1,1.0,1)         //proporion of marketable legal for length 4 class since 2005
  vector log_rec(1,ny)						//Annual recruitment log scale
...
//  vector ete(1,ny)                              //estimated total annual effort: summer fishery
  ...
  vector tscd(1,ny)                             //Winter subsitence catch discarded
...
//matrix sel(1,na,1,ny-1)							//selectivity of summer fishery combined
...
//===============================================================================================================
// 4.4  Calculate Winter subsistence discards
...
//
  for (i=1;i<=ny;i++)
	...
	if (tscd(i) > 0)
		{
		  tt0 += tscd(i)/tsc(i);  // Sum proprotion of discards
...
// working variables: Selectivity
  dvariable sc1,sc2,sc3,sc4, sc5, sc6, st1,st2,sw1,sw2;
...
//     mp3(j) = 1.0-1.0/(1.0+mfexp(-1.0*mo1*(pp-mo2)));
	 ...
	 sel08(j) = 1.0/(1.0+mfexp(-1.0*sc5*(pp-sc6)));  // commercial pot selectivity 2008 - : logistic function//
     selw(j) = 1.0/(1.0+mfexp(-1.0*sw1*(pp-sw2)));   // winter pot selectivity 1981-2011: logistic function//
     selt(j) = 1.0/(1.0+mfexp(-1.0*st1*(pp-st2)));   // summer trawl net selectivity: logistic function//
...
// Molting probability: Molting probability of the first length class is adjusted to be 1.
   mp1 = mp1/mp1(1);
   selt = selt/selt(na-1);   // Trawl survey
...
//===============================================================================================================
// 7.0  Function:  Estimate the First Year abundance and length composition
...
// Assign first year abundance on Feb 1st
   for (j=1;j<=na;j++)
	{
...
//===============================================================================================================
// 8.1 Winter commercial catch and discards size composition are dealt the same way as summer commercial
//===============================================================================================================
...
// Calculate proportiojn of new shell and old shell in winter commercialcatch   	
    for (j=1;j<=na;j++)
     {
	...
	for (j=1;j<=na;j++)
     {
      tt1 += (npw(j,i)+opw(j,i))*lg(j); // For commercial Catch 	
     }
...
//===============================================================================================================
    ...
    for (j=1;j<=na;j++)
	{
     nps(j,i) = (npw(j,i)-enwp(j,i)*tsc(i)-enwc(j,i)*twc(i)-(twc(i)/tt1)*(npw(j,i)+opw(j,i))*selw(j)*(1-lg(j))*hm(2)-tscd(i)*enwpd(j,i)*hm(2))*exp(-0.417*Mn(j));
     ops(j,i) = (opw(j,i)-eowp(j,i)*tsc(i)-eowc(j,i)*twc(i)-(twc(i)/tt1)*(npw(j,i)+opw(j,i))*selw(j)*(1-lg(j))*hm(2)-tscd(i)*eowpd(j,i)*hm(2))*exp(-0.417*Mo(j));	
...
// Calculate total number of legal crab on July 1st
...
//  cout <<"Test 21."<<endl;
// Crab abundance right after summer commercial fishery 	
    for (k=1;k<=na;k++)
     {	
...
// Calculate New Shell popululation abundance by length class: Molting
    for (j=1;j<=na;j++)
     {
      pp = 0.0;
      for (k=1;k<=j;k++) pp += gr(k,j)*tt0(k)*mp0(k); //New shell crab after molting
//  cout <<"Test 23."<<endl;
//      npw(j,i+1) = pp*exp(-(0.583-ys(i))*Mn(k));             //error: Need to change Mn(k) to Mn(j)
      npw(j,i+1) = pp*exp(-(0.583-ys(i))*Mn(j));
//  cout <<"Test 24."<<endl;
// Add Recruit to New shell crab
...
//  cout <<"Test 3."<<endl;
  // at the last_phase, calculated abundances in last year & legal abundance for all years.
...
//===============================================================================================================
// 8.7  Calculate legal Crab of Jul 1st of the projected year
//===============================================================================================================
  last_legal = 0.0;
  for (j=1; j<=na; j++) last_legal += last_y(j)*exp(-0.417*Mn(j))*lg(j)*wm(j)*sel08(j);
  last_subl = 0.0;
  for (j=1; j<=na; j++) last_subl += last_y(j)*exp(-0.417*Mn(j))*(1-lg(j))*wm(j)*sel08(j);


//  last_legal = last_y(3)*exp(-0.417*Mn(3))*lg(3)*wm(3)*sel08(3)+last_y(4)*exp(-0.417*Mn(4))*lg(4)*p4*wm(4)*sel08(4)+last_y(5)*exp(-0.417*Mn(5))*lg(5)*wm(5)*sel08(5)+last_y(6)*exp(-0.417*Mn(6))*lg(6)*wm(6)*sel08(6);
//  last_subl = last_y(1)*(1-lg(1))*wm(1)*sel08(1)+last_y(2)*(1-lg(2))*wm(2)*sel08(2)+last_y(3)*(1-lg(3))*wm(3)*sel08(3)+last_y(4)*(1-lg(4))*wm(4)*sel08(4)+last_y(5)*(1-lg(5))*wm(5)*sel08(5)+last_y(6)*(1-lg(6))*wm(6)*sel08(6);
  last_ofl = last_legal*(1.0-exp(-M));
  last_mmb = 0.0;
  for (j=1; j<=na; j++) last_mmb += (npw(j,ny+1)+opw(j,ny+1))*wm(j)*exp(-0.417*Mn(j));
...
// Estimate Survery Q for NOAA trawls
		if(it(i) < scy(1)) {ettq(i) = qtno*(ett(i));}
			else
// Estimate Survery Q for ADF&G trawls			
		ettq(i) = qtad*(ett(i));
   }
      ...
      for (k=1;k<=na;k++)
       {
        ...
        for (kk=1;kk<=k;kk++)
        {
          tt0 = (ref_ps(kk,i)*mfexp(-ref_ys*Mn(kk))-ref_catch_s(kk,i)-ref_Hts*ref_ps(kk,i)*sel(kk,ny-1)*(1.0-lg(kk))*hm(1))*mfexp(-(0.583-ref_ys)*Mn(kk));
          if (tt0<0.0) tt0 = 0.0;
        ...
        ref_opw(k,i) = (ref_ps(k,i)*mfexp(-ref_ys*Mn(k))-ref_catch_s(k,i)-ref_Hts*ref_ps(k,i)*sel(k,ny-1)*(1.0-lg(k))*hm(1))*mfexp(-(0.583-ref_ys)*Mn(k))*(1.0-mp0(k));
