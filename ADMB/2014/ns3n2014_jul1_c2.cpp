...
void model_parameters::evaluate_the_objective_function(void)
{
  ...
  tf(7) = -(sum(elem_prod(sc,colsum(elem_prod(onc+ooc,log(enc+eoc+1.e-3))))) - sum(elem_prod(sc,colsum(elem_prod(onc+ooc,log(onc+ooc+1.e-3))))));      
  //Log likelhihood size proportion for summer fishery survey    
  tf(9) = -(sum(elem_prod(so,colsum(elem_prod(ono+ooo,log(eno+eoo+1.e-3))))) - sum(elem_prod(so,colsum(elem_prod(ono+ooo,log(ono+ooo+1.e-3))))));  
