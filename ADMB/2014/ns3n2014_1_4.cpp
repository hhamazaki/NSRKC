 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  ms.allocate(0.1,5.0,ms_phase,"ms");
...
void model_parameters::initializationfunction(void)
{
  ...
  log_initpop.set_initial_value(8.0);
}
...
void model_parameters::evaluate_the_objective_function(void)
{
  ...
  f = tf1;
  tf2 = sum(elem_prod(st,colsum(elem_prod(ont,log(ent+1.e-3)))))-sum(elem_prod(st,colsum(elem_prod(ont,log(ont+1.e-3)))));   
  f -=tf2;   
   ...
   f += tf3;   
  tf4 = sum(elem_prod(sc,colsum(elem_prod(onc+ooc,log(enc+eoc+1.e-3)))))-sum(elem_prod(sc,colsum(elem_prod(onc+ooc,log(onc+ooc+1.e-3)))));      
  f -= tf4;
  tf5 = sum(elem_prod(so,colsum(elem_prod(ono+ooo,log(eno+eoo+1.e-3))))) - sum(elem_prod(so,colsum(elem_prod(ono+ooo,log(ono+ooo+1.e-3)))));      
  f -= tf5;  
  tf6 = norm2(log_relrec)/(2*SDRec*SDRec);                       
  f += tf6;
  tf7 = lamw*(sum(elem_prod(sw,colsum(elem_prod(onw+oow,log(enw+eow+1.e-3)))))-sum(elem_prod(sw,colsum(elem_prod(onw+oow,log(onw+oow+1.e-3))))));      
  f -= tf7;
}
...
void model_parameters::report()
{
 adstring ad_tmp=initial_params::get_reportfile_name();
  ...
  report << "tf1" << endl << tf1 << " " << tf2 << " " << tf3 << " " << tf4 << " " << tf5 << " " << tf6 << " " << tf7 << endl;
}
