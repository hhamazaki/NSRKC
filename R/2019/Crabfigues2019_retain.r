...
if(dat$base[2]==-1) plot(length.c, moltp,type='s',ylim=c(0,1), main='Molting Probability')
#---------------------------------------------------------------------------
#   4.1.2  Plot Trawl Selectivity   
#--------------------------------------------------------------------------- 
trawlpn <- c(rept$selt[1,],rept$selt[1,dat$na])
...
RMSE.trawl <- sqrt(sum((log(dat$tt)-log(rept$ettq))^2)/length(dat$tt))
...
#RMSE.pot
RMSE.cpue
resd.trawl <- dat$tt - rept$ettq
resd.pot <- dat$tp - (rept$etp1+rept$etp2)
resd.cpue <- dat$stcpue[-c(1,13,16)] - rept$ecpue[-c(1,13,16)]
...
points(dat$it[c(7:11,13:15,17)], dat$tt[c(7:11,13:15,17)]/1000, pch=21, bg='red')
# Model estiamted trawl survey abundance

points(dat$it,rept$ettq/1000,pch=19, col=4)

#lines(year,total.abundance[1:ny],lwd=2)
legend("topright", pch = c(1,19,19), col= c(1,2,4), 
       legend = c('Observed: NMFS','Observed: ADFG','Predicted'),bg='white')
