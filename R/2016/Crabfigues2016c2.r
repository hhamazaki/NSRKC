...
pdffile <- paste(admodeldir,'ns3n2016.pdf',sep='')
...
##############################################################################################################
# Calculate total 
  st <- colSums(dat$ont)+colSums(dat$oot)
  sw <- colSums(dat$onw)+colSums(dat$oow)
  sc <- colSums(dat$onc)+colSums(dat$ooc)
  so <- colSums(dat$ono)+colSums(dat$ooo)

  ont <- dat$ont/st[col(dat$ont)]
  oot <- dat$oot/st[col(dat$oot)]
  onw <- dat$onw/sw[col(dat$onw)]
  oow <- dat$oow/sw[col(dat$oow)]
  onc <- dat$onc/sc[col(dat$onc)]
  ooc <- dat$ooc/sc[col(dat$ooc)]
  ono <- dat$ono/so[col(dat$ono)]
  ooo <- dat$ooo/so[col(dat$ooo)]
...
sw3 <- par1[which(par1$name == 'sw3'),3]
sc1 <- par1[which(par1$name == 'log_sc1'),3]
st1 <- par1[which(par1$name == 'log_st1'),3]

selc <-function(log.par){
sel <- 1/(1+exp(exp(log.par)*(max(sizeclass)-sizeclass)+log(1.0/0.999-1.0)))
sel
}
moltp <- 1-1/(1+exp(exp(pars$log_mo1)*(min(sizeclass)-sizeclass)+log(1.0/0.001-1.0)))
plot(length.c, moltp,type='s', main='Molting Probability', ylim=c(0,1))
trawlpn <- selc(st1[1])
plot(length.c, trawlpn,type='s', main='Trawl Selectivity', ylim=c(0,1))
trawlpa <- selc(st1[2])
#plot(length.c, trawlpa,type='s', main='ADFG Trawl Selectivity', ylim=c(0,1))
...
#wpotp(6:7) <- (pars$sw3)
wpotp <- 1-1/(1+exp(exp(pars$log_sw1)*(sizeclass[1]-sizeclass)+log(1.0/0.001-1.0)))
wpotp[1:dat$msn[2]] <- sw3

plot(length.c, wpotp,type='s', main='Winter pot Selectivity', ylim=c(0,1))
sc1p <- selc(sc1[1])
plot(length.c, sc1p,type='s', main='Commercial Selectivity', ylim=c(0,1))
sc2p <- selc(sc1[2])
#plot(length.c, sc2p,type='s', main='Ctommercial 93-05 Selectivity', ylim=c(0,1))
...
lines(c(0,dat$maxs[1]),c(0,dat$maxs[1]))
...
lines(c(0,dat$maxs[2]),c(0,dat$maxs[2]))
title(xlab = 'Input', line = 2)
title(ylab = 'Implied', line = 2)
# Plot Implied vs. year
...
lines(c(0,max(nrecap)),c(0,max(nrecap)))
title(xlab = 'Input', line = 2)
title(ylab = 'Implied', line = 2)
...
ciwl <- exp(log(dat$tt/1000)*(1+2*dat$cv))
ciwu <- exp(log(dat$tt/1000)*(1-2*dat$cv))
plot(dat$it,dat$tt/1000, ylab = 'Crab Abundance (million)', xlab = 'Year', ylim= c(0,8),bty='l')
...
#FOFL <- ifelse(cofl>1,pars$M,ifelse(0.25<cofl&&cofl<=1,pars$M*(cofl-0.1)/0.9,0))
...
hrate <- tcatch/rept$Legal[-length(rept$Legal)]
...
mtext("Discards length: observed vs predicted", side=3, line = -24, outer=T)
...
mtext("1993-2014: Recovery after 1 year", cex = 0.9,  side=3, line = -30.5, outer=T)
for (i in 1:(dat$na-1)){
x <- plot(len.c,tagrecap22[i,],ylim=c(0,1),pch=19, cex=0.8, tck = -0.05, bty='l',ylab = '', xlab = '')
lines(len.c,rept$egr22[i,],lty=2)
text(len.c[3],0.9,paste(len.c[i],'mm'),cex=1.0)
}
mtext("1993-2014: Recovery after 2 years", cex = 0.9,  side=3, line = -40.5, outer=T)
for (i in 1:(dat$na-1)){
x <- plot(len.c,tagrecap32[i,],ylim=c(0,1),pch=19, cex=0.8, tck = -0.05, bty='l',ylab = '', xlab = '')
lines(len.c,rept$egr32[i,],lty=2)
text(len.c[3],0.9,paste(len.c[i],'mm'),cex=1.0)
}
mtext("1993-2014: Recovery after 3 years", cex = 0.9,  side=3, line = -50.5, outer=T)
