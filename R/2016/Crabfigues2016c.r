...
infn <- paste(datadir,'ns3n2016Rtag.txt',sep='')
# outfn: output file name  

##############################################################################################
#   Activate either, based on model you use 
###############################################################################################
outfn <- paste(datadir,'ns3n2016R.dat',sep='')
...
prefix <- paste(admodeldir,'ns3n2016_Feb1_1',sep='')
...
plot(lengthclass, trawlpn,type='s', main='Trawl Selectivity', ylim=c(0,1))
...
plot(lengthclass, sc1p,type='s', main='Commercial Selectivity', ylim=c(0,1))
...
tefnn <- ifelse(dat$st*dat$efnt>dat$maxss,dat$maxss,dat$st*dat$efnt)
...
scefn <-scefn[-c(1,16)]
scefnn <- ifelse(dat$sc*dat$efnw>dat$maxss/2,dat$maxss/2,dat$sc*dat$efnw)[-c(1,16)]

#Winter Pot survey effective sample size
wpp <- rept$enw+rept$eow
wp <- dat$onw+dat$oow
wpefn <-colSums(wpp*(1-wpp))/colSums((wpp-wp)^2)
wpefnn <- ifelse(dat$sw*dat$efnw>dat$maxss/2,dat$maxss/2,dat$sw*dat$efnw)
...
for(i in 1:21) 
  {
        tagrecap1[dat$tag_recov1[i,1],dat$tag_recov1[i,2]] <- dat$tag_recov1[i,3]+dat$tag_recov2[i,3]
        tagrecap2[dat$tag_recov1[i,1],dat$tag_recov1[i,2]] <- dat$tag_recov1[i,4]+dat$tag_recov2[i,4]
        tagrecap3[dat$tag_recov1[i,1],dat$tag_recov1[i,2]] <- dat$tag_recov1[i,5]+dat$tag_recov2[i,5]
...
opefnn <- ifelse(dat$so*dat$efnw>dat$maxss/2,dat$maxss/2,dat$so*dat$efnw)
...
plot(tefnn,tefn, main=tmain, xlim = c(0,dat$maxss),ylim=c(0,max(tefn)))
...
plot(dat$it+1975, tefn,main=tmain)
...
title(ylab = 'Frequency', line = 2)
# plot Input vs. Implied
plot(scefnn, scefn, main=tmain, xlim = c(0,dat$maxss/2),ylim=c(0,max(scefn)))
lines(scefn~scefnn-1,lty=2)
lines(c(0,dat$maxss/2),c(0,dat$maxss/2))
title(xlab = 'Input', line = 2)
title(ylab = 'Implied', line = 2)
# Plot Implied vs. year
plot(year[-c(1,16,dat$ny+1)], scefn,main=tmain)
title(xlab = 'Year', line = 2)
title(ylab = 'Implied', line = 2)
...
title(ylab = 'Frequency', line = 2)
# plot Input vs. Implied
plot(wpefnn, wpefn, main=tmain, xlim = c(0,dat$maxss/2),ylim=c(0,max(wpefn)))
abline(lm(wpefn~wpefnn-1),lty=2)
lines(c(0,dat$maxss/2),c(0,dat$maxss/2))
title(xlab = 'Input', line = 2)
title(ylab = 'Implied', line = 2)
# Plot Implied vs. year
plot(dat$iw+1975, wpefn, main=tmain)
title(xlab = 'Year', line = 2)
title(ylab = 'Implied', line = 2)
...
title(ylab = 'Frequency', line = 2)
# plot Input vs. Implied
plot(opefnn, opefn, main=tmain, xlim = c(0,dat$maxss/2),ylim=c(0,max(opefn)))
abline(lm(opefn~opefnn-1),lty=2)
lines(c(0,dat$maxss/2),c(0,dat$maxss/2))
title(xlab = 'Input', line = 2)
title(ylab = 'Implied', line = 2)
# Plot Implied vs. year
plot(dat$io+1975, opefn, main=tmain)
title(xlab = 'Year', line = 2)
title(ylab = 'Implied', line = 2)
