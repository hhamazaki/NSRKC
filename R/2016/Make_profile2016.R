...
infn <- paste(indatadir,'ns3n2016Rtagc3.txt',sep='')
datadir <- paste('C:/Projects/Norton_Sound/NSCrab/Prediction_model/2016/profile/')
datname <- paste('ns3n2016_M.dat')
...
#maxgrad[i] <- test1[16]
MMB[i,] <- as.double(scan(fn,skip=97,nlines=1,nmax=(ny +1),quiet=T,what="")) 
M.prof[i] <- as.double(scan(fn,skip=111,nlines=1,quiet=T,what="")) 
tf1.prof[i,] <- as.double(scan(fn,skip=113,nlines=1,quiet=T,what="")) 
...
lines(x.axis, tf1.prof[,4]+tf1.prof[,5]-min(tf1.prof[,4]+tf1.prof[,5],na.rm=TRUE),lty=3,lwd=2)
