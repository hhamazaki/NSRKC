...
infn <- paste(datadir2,'ns3n2014Rtag2.txt',sep='')
...
tf1.prof <- matrix(0,ncol=12,nrow=nrep)
...
#maxgrad[i] <- test1[16]
MMB[i,] <- as.double(scan(fn,skip=90,nlines=1,nmax=(dat$ny +1),quiet=T,what="")) 
M.prof[i] <- as.double(scan(fn,skip=100,nlines=1,quiet=T,what="")) 
tf1.prof[i,] <- as.double(scan(fn,skip=102,nlines=1,quiet=T,what="")) 
...
plot(M, tf1.prof[,6]+tf1.prof[,5],type='l',main='Winter Pot length comp')
plot(M, tf1.prof[,7]+tf1.prof[,8],type='l',main='Summer Commercial length comp')
plot(M, tf1.prof[,9]+tf1.prof[,10],type='l', main='recruits')	
plot(M, tf1.prof[,11],type='l',main='Observer length comp')
plot(M, tf1.prof[,12],type='l',main='Tag recovery')

# Plot likelihood profile standardized 
par(mfrow=c(1,1),mar = c(5, 5, 5, 5))
plot(M, M.prof-min(M.prof,na.rm=TRUE),type='l', lwd=2, main='Total negative log likelihood', ylab='Likelihood',ylim=c(0,8))
...
lines(M, tf1.prof[,5]+tf1.prof[,6]-min(tf1.prof[,6]+tf1.prof[,5],na.rm=TRUE),lty=6,col=2)
lines(M, tf1.prof[,7]+tf1.prof[,8]-min(tf1.prof[,7]+tf1.prof[,8],na.rm=TRUE),lty=2,lwd=2,col=4)
lines(M, tf1.prof[,9]+tf1.prof[,10]-min(tf1.prof[,9]+tf1.prof[,10],na.rm=TRUE),lty=4,col=4)
lines(M, tf1.prof[,11]-min(tf1.prof[,11],na.rm=TRUE),lty=5,col=4)
lines(M, tf1.prof[,12]-min(tf1.prof[,12],na.rm=TRUE),lty=6,col=4)
