...
plot(lengthclass, sc2ps,type='s', main='Commercial 93-07 Selectivity', ylim=c(0,1))
sc3p <- 1/(1+exp(-(exp(pars$log_sc5)*(sizeclass-exp(pars$log_sc6)))))
sc3ps <- ifelse(sc3p/sc3p[5]>1,1,sc3p/sc3p[5])
plot(lengthclass, sc3ps,type='s', main='Commercial 08-12 Selectivity', ylim=c(0,1))
...
plot(year[1:dat$ny], total.abundance[1:dat$ny], ylab = 'Abundance (million crabs)', typ = 'l', lwd=2, xlab = 'Year',ylim= c(0,10),bty='l')
legal.abundance <- colSums(rept$nps[4:6,]+rept$ops[4:6,])
lines(year[1:dat$ny], legal.abundance[1:(dat$ny)]/1000, lty=2)
recruit.abundance <- colSums(rept$nps[1:2,]+rept$ops[1:2,])
lines(year[1:dat$ny], recruit.abundance[1:(dat$ny)]/1000, lty=3)
...
# Figure 3. cpue data 
ciw <- 2*dat$stcpuese
plotCI(year[-(dat$ny+1)], dat$stcpue, uiw=ciw, ylab = 'CPUE',  xlab = 'Year', ylim=c(0,7), gap=0.2, bty='l')
...
test2 <- subset(test,((test$years != 1976)&(test$years != 1991)&(test$years != 2013)))
bubbleplot()
